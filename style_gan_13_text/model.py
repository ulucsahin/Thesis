"""
Implementation of NVidia's StyleGAN.
...
class Intermediate_Generator(nn.Module):
    ...
    def __init__(self, n_fc, dim_latent):
        ...
        layers = [PixelNorm()]
        for i in range(n_fc):
            layers.append(SLinear(dim_latent, dim_latent)) # dim latent is 300
...
class StyleBased_Generator(nn.Module):
    ...
    def __init__(self, n_fc, dim_latent, dim_input):
        ...
        self.convs = nn.ModuleList([
            Early_StyleConv_Block(600, dim_latent, dim_input),
            StyleConv_Block(600, 512, dim_latent),
    ...
    def forward(self, latent_z,
        ...
        if latent_w_center is not None:
            latent_w = [latent_w_center + psi * (unscaled_latent_w - latent_w_center) for unscaled_latent_w in latent_w]

        # Gene"rate" needed Gaussian noise
        ...
        for i, conv in enumerate(self.convs):
            ...
            if i > 0 and step > 0:
                result_upsample = nn.functional.interpolate(result, scale_factor=2, mode='bilinear', align_corners=False)
...
class Discriminator(nn.Module):
    ...
    def __init__(self):
        ...
        self.from_rgbs = nn.ModuleList([
            SConv2d(channels, 16, 1),
            SConv2d(channels, 32, 1),
            SConv2d(channels, 64, 1),
            SConv2d(channels, 128, 1),
            SConv2d(channels, 256, 1),
            SConv2d(channels, 512, 1),
            SConv2d(channels, 512, 1),
            SConv2d(channels, 512, 1),
            SConv2d(channels, 512, 1)
        ])
        ...
        self.embedding_shapers = nn.ModuleList([
            SLinear(300, 1), # resolution not supported
            SLinear(300, 1), # resolution not supported
            SLinear(300, 256*256),
            SLinear(300, 128*128),
            SLinear(300, 64*64),
    ...
    def forward(self, image, embedding,
        ...
        embedding = self.embedding_shapers[self.n_layer - step - 1](embedding)
        embedding = embedding.view([embedding.shape[0], 1, image.shape[2], image.shape[3]]) # [batch_size, 1, image_size, image_size]

        image = torch.cat([image, embedding], 1)
        ...
        for i in range(step, -1, -1):
            ...
            if i == step:
                result = self.from_rgbs[layer_index](image) # [8, 512, 8, 8]
        ...
        # Convert it into [batch, channel(512)], so the fully-connected layer could process it.

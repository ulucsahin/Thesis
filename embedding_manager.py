"""
This file has 3 word-sentence embedders and 1 image embedder.
Currently class named "Embedder" is being used.
"""

from external_libs.InferSent.models import InferSent
import sister
import torch
...
from style_gan_13_text.model import conv1x1
import torch.nn.functional as F


class Embedder(object):
    """
    Sentence embedder which returns 300 dimension embeddings.
    Update: Now uses Sister library which uses FastText.
    """
    def __init__(self):
        self.setup_model()
...
# Different version (akanimax version)
...
class RNN_ENCODER(torch.nn.Module):
    ...
    def define_module(self):
        self.encoder = torch.nn.Embedding(self.ntoken, self.ninput)
        self.drop = torch.nn.Dropout(self.drop_prob)

        if self.rnn_type == 'LSTM':
            # dropout: If non-zero, introduces a dropout layer on
            # the outputs of each RNN layer except the last layer
            self.rnn = torch.nn.LSTM(self.ninput, self.nhidden,
                               self.nlayers, batch_first=True,
                               dropout=self.drop_prob,
                               bidirectional=self.bidirectional)

        elif self.rnn_type == 'GRU':
            self.rnn = torch.nn.GRU(self.ninput, self.nhidden,
                              self.nlayers, batch_first=True,
                              dropout=self.drop_prob,
                              bidirectional=self.bidirectional)
